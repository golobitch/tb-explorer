...
The body is generated by `scripts/release-notes.sh`, not by GitHub. GitHub picks the previous
release by date rather than by tag prefix, so a `cli-v*` release would baseline against whichever
`ui-v*` release came last and list its commits too.

What it publishes: a paragraph introducing the release, then the commits since the previous tag
**of the same deliverable** that touched that deliverable's paths, grouped by commitizen type.
Features, fixes and docs get headings; ci, chores, tests and refactors go in a collapsed block.
Which paths belong to which deliverable is one `case` near the top of the script — a new top-level
directory belongs to nobody until it is added there.

Run it before you tag, to see what a release would say:

```sh
scripts/release-notes.sh cli-v0.2.0 --since cli-v0.1.0 --to HEAD
scripts/release-notes.sh cli-v0.2.0 --no-summary          # skip the model call
```

To fix the notes on a release that has already published, regenerate and upload — the tag and the
assets are untouched:

```sh
scripts/release-notes.sh cli-v0.2.0 --out notes.md
gh release edit cli-v0.2.0 --notes-file notes.md
```

The opening paragraph is written by Claude from that commit list and labelled as such on the page.
It publishes unread, so if it says something you would rather it did not, edit the release body —
it is a text box, and the assets do not care.
