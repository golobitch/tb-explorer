...
Prerequisites: Xcode 16.3+ and [XcodeGen](https://github.com/yonaskolb/XcodeGen) (`brew install xcodegen`). The Xcode project is generated from `ui/project.yml` and isn't committed. Every `make` target runs from the repo root.
...
Sources/TBKit/              read-only Swift client over tb_client
  ...
  Export.swift              CSV/JSON encoding, shared with the Raw tab
...
Shared with the other front ends, one level up: `Vendor/tigerbeetle/` (the pinned client),
`scripts/` (dev cluster, icon rendering), `docs/images/` and the `Makefile`.
